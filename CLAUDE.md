...
| **Native Sonos SMAPI** | Done | `server/sonos/` - Complete SMAPI implementation for direct Sonos integration (no bonob needed) |
...
### Native Sonos SMAPI (NEW)
- `server/sonos/smapi.go` - Main SOAP router and handler
- `server/sonos/types.go` - SMAPI request/response types and SOAP envelope structures
- `server/sonos/auth.go` - Device linking authentication flow with encrypted token storage
- `server/sonos/browsing.go` - getMetadata, getMediaMetadata, getMediaURI handlers
- `server/sonos/search.go` - Search functionality
- `server/sonos/discovery.go` - SSDP device discovery and registration
- `model/sonos_device_token.go` - Device token model and repository interface
- `persistence/sonos_device_token_repository.go` - Database persistence for device tokens
- `db/migrations/20251230120000_create_sonos_device_token.go` - Database migration
- `server/nativeapi/sonos_devices.go` - API endpoints for device management

**Configuration Options** (in `navidrome.toml` or environment variables):
```toml
[Sonos]
Enabled = true           # Enable/disable Sonos SMAPI (default: false)
ServiceName = "Navidrome"  # Service name shown in Sonos app
AutoRegister = false     # Auto-discover and register with Sonos devices
```

**Environment Variables**:
- `ND_SONOS_ENABLED=true`
- `ND_SONOS_SERVICENAME="My Music Server"`
- `ND_SONOS_AUTOREGISTER=false`

**API Endpoints** (when Sonos is enabled):
- `GET /api/sonos/devices` - List linked Sonos devices for current user
- `DELETE /api/sonos/devices/{id}` - Revoke a linked device

**HTTPS/TLS Notes**:
For Sonos integration, HTTPS is strongly recommended. You have two options:
1. **Reverse Proxy (Recommended)**: Use Traefik, Nginx, or similar to terminate TLS
2. **Direct TLS**: Configure `TLSCert` and `TLSKey` in Navidrome config
